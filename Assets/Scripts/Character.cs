...
public class Character : MonoBehaviour
{
    ...
    private float PhysicalExpense = 0.4f;
    ...
    */
    ...
    {
        // Set PhysicalExpense to 0.2 while health power-up is active
        PhysicalExpense = 0.2f;

        // Reset the slider value to full (100%)
        healthSlider.value = 1f;

        // Activate the slider
        healthSlider.gameObject.SetActive(true);

        // Set the flag to true (power-up is active)
        isHealthInsuranceActive = true;

        // Stop any existing depletion coroutine to prevent multiple instances running
        if (depleteCoroutine != null)
        {
            StopCoroutine(depleteCoroutine);
        }

        // Start a new coroutine to deplete the slider over time
    ...
    private IEnumerator DepleteHealthOverTime()
    {
        ...
        // Gradually deplete the slider value over the power-up duration
        while (elapsedTime < powerUpDuration)
        {
            elapsedTime += Time.deltaTime;
            healthSlider.value = Mathf.Lerp(startingValue, 0, elapsedTime / powerUpDuration);
            yield return null; // Wait for the next frame
        }

        // Ensure the slider is set to 0 at the end
        healthSlider.value = 0;

        // Deactivate the slider after it fully depletes
        healthSlider.gameObject.SetActive(false);

        // Reset PhysicalExpense back to 0.4 after power-up ends
        PhysicalExpense = 0.4f;

        // Reset the slider value to 1 (full) after deactivating
        healthSlider.value = 1f;

        // Set the flag to false (power-up duration is over)
        isHealthInsuranceActive = false;

        // Stop tracking the coroutine since it has finished
    ...
    {
        HandleMutualRisesValue = (int)(Assets.MutualFundsPortfolio * 0.04f); // 10 percent of the current number of coins
        GameManager.numberOfCoins += HandleMutualRisesValue;
        string formattedRepairValue = NumberFormatter.FormatNumberIndianSystem(HandleMutualRisesValue);

        popupManager.ShowPopup($"+{HandleMutualRisesValue}", Color.green, new Vector3(PopUpX, PopUpY, PopUpZ));
        Destroy(other.gameObject);
    }
    ...
    public void HandlePhysicalHit(Collider other)
    {
        ...
        {
            Debug.Log("Already hit a physical object. Ignoring further deductions.");
            return; // Ignore if already hit
        }

        if (GroundSpawnerTest.isTutorialMode) // Assuming you have access to this property
        {
            Debug.Log("Tutorial mode is active. No physical expense applied.");
        ...
        Debug.Log($"Initial deduction: {HandlePhysicalHitValue} coins");
    ...
    private IEnumerator DelayedHitEffect()
    {
        ...
        Debug.Log($"Coins after deduction: {GameManager.numberOfCoins}");
    ...
    {
        Debug.Log("OnTriggerEnterPhysicalRight called with collider: " + other.name);
    ...
    {
        Debug.Log("OnTriggerEnterPhysicalLeft called with collider: " + other.name);
        if (!isFlying)
        {
    ...
    {
        Debug.Log("OnTriggerStay called with collider: " + other.name);
    ...
    {
        if (other.CompareTag("PBRE"))
        {
            // Reset the flag when leaving the trigger area
